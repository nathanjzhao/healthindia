...
from user_history import load_user_history, save_user_history, add_entry_to_history, update_user_info
...
def interpret_response(user_response, question_node):
    ...
    Return only the matching option or "invalid" without explanation.
    """
    
    interpreted_response = generate_openai_response(prompt)
    print(options)
    print(interpreted_response)
    return interpreted_response.strip().lower()
...
def update_user_history(question, answer, user_history):
    ...
    """
    gpt_response = generate_openai_response(prompt)
    bullet_points = [line.strip() for line in gpt_response.strip().split('\n') if line.strip()]
...
def terminal():
    ...
    rephrased_question = rephrase_question(decisionTree["root"]["question"], "", False, user_history)
